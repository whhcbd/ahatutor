# AhaTutor - 遗传学可视化交互解答平台

> 基于 AI 的遗传学学习平台，实现"自然语言输入 + AI理解 + 实时可视化 + 交互探索"，打造真正的"顿悟时刻"(Aha Moment)

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![TypeScript](https://img.shields.io/badge/TypeScript-5.5-blue)](https://www.typescriptlang.org/)
[![React](https://img.shields.io/badge/React-18-61DAFB)](https://react.dev/)
[![NestJS](https://img.shields.io/badge/NestJS-10-E0234E)](https://nestjs.com/)

---

## 项目概述

**AhaTutor** 是一个创新的学科学习平台，通过六 Agent 协作流水线，将复杂的概念转化为：

- 📊 **可视化知识图谱** - 一眼看穿概念关系
- 📖 **生动学习叙事** - 故事化讲解，记忆深刻
- ❓ **智能题目生成** - 精准练习，举一反三
- 🌐 **联网资源推荐** - 最新资讯，扩展视野

**MVP 学科**: 遗传学

---

## 功能实现状态

### ✅ 已完成功能

| 模块 | 状态 | 说明 |
|------|:------:|------|
| **六 Agent 流水线** | ✅ | 完整的概念分析到题目生成流程 |
| **RAG 框架** | ✅ | 向量检索、混合查询、文档解析 |
| **LLM 多管道** | ✅ | OpenAI/Claude/DeepSeek/GLM，流式输出 |
| **速通模式前端** | ✅ | 完整刷题流程（出题→答题→判断→自评→解析） |
| **知识图谱** | ✅ | Neo4j + D3.js 力导向图 |
| **错题管理** | ✅ | OCR 识别 + 举一反三变式练习 |
| **学情报告** | ✅ | 数据统计 + ECharts 图表 |
| **Agent Skills** | ✅ | 联网搜索 + 资源推荐 |
| **ReportModule** | ✅ | 已注册到主应用 |

### ⏳ 待完成功能

| 模块 | 状态 | 说明 |
|------|:------:|------|
| **VisualDesigner** | ✅ | 可视化设计 Agent |
| **NarrativeComposer** | ✅ | 叙事作曲 Agent |
| **AuthModule** | ⏳ | 用户认证模块 |
| **QuizModule** | ⏳ | 智能组卷服务 |
| **CacheModule** | ⏳ | Redis 缓存优化 |

---

## 六 Agent 协作流水线

```
用户输入 "伴性遗传"
    ↓
┌─────────────────────────────────────────┐
│ Agent 1: ConceptAnalyzer                │ ✅
│ "这真正在问什么？"                        │
│ → 结构化概念分析（领域、复杂度、可视化潜力）│
└──────────────┬──────────────────────────┘
               ↓
┌─────────────────────────────────────────┐
│ Agent 2: PrerequisiteExplorer ⭐         │ ✅
│ "理解这个前需要先懂什么？"                 │
│ → 遗传学知识树（递归分解到基础概念）        │
└──────────────┬──────────────────────────┘
               ↓
┌─────────────────────────────────────────┐
│ Agent 3: GeneticsEnricher               │ ✅
│ "精确的遗传学原理是什么？"                 │
│ → 遗传学公式、定律、实验、例子             │
└──────────────┬──────────────────────────┘
               ↓
┌─────────────────────────────────────────┐
│ Agent 4: VisualDesigner                 │ ✅
│ "如何展示这个概念？"                       │
│ → 知识图谱可视化、动画设计、颜色方案        │
└──────────────┬──────────────────────────┘
               ↓
┌─────────────────────────────────────────┐
│ Agent 5: NarrativeComposer              │ ✅
│ "连接这些概念的故事是什么？"               │
│ → 学习路径、讲解顺序、知识点串联           │
└──────────────┬──────────────────────────┘
               ↓
┌─────────────────────────────────────────┐
│ Agent 6: QuizGenerator                  │ ✅
│ "如何生成题目和评估？"                    │
│ → 选择题、填空题、解析、等级系统           │
└──────────────┬──────────────────────────┘
               ↓
         完整的学习体验
```

---

## 核心功能详解

### 1. 速通模式（短时记忆）

**目标用户**: 考前突击、快速复习、即时答疑

```
AI 出题 → 用户回答 → AI 判断 → 用户自评 → 分级解析 → 继续下一题
    ↓                                                        ↓
 低级错误（笔误）→ 继续刷题         高级错误（知识性）→ 进入解答模式
```

### 2. 深度模式（长时记忆）

**目标用户**: 系统学习、深度研究、长期规划

**核心创新：反向知识树**
```
用户输入："解释伴性遗传"
  ↓
🌳 反向递归分解：
  "理解伴性遗传前需要先懂什么？"
    → 伴性遗传前需要：孟德尔定律、性染色体
      → 孟德尔定律前需要：基因型/表型、分离定律
        → 基因型前需要：基因概念、染色体基础 ✓
  ↓
🎯 从基础向上构建学习路径
```

### 3. Agent Skills

| Skill | 功能 | API |
|-------|------|-----|
| **联网搜索** | 实时检索最新遗传学资讯 | `POST /agent/skills/search` |
| **资源推荐** | 根据学习进度推荐视频/文献 | `POST /agent/skills/resources` |

### 4. 错题管理

- 📷 拍照上传错题（调用设备相机）
- 🔍 OCR 自动识别题目内容
- 🔄 举一反三：基于错题自动生成相似题
- 📁 错题分类存储和个人错题库

### 5. 学情报告

- 📊 日/周/专题报告
- 📉 薄弱点分析、错误原因统计
- 🕸️ 知识点掌握度雷达图

---

## 技术栈

### 前端层

| 技术 | 版本 | 用途 |
|------|------|------|
| React | 18 | UI 框架 |
| TypeScript | 5.5 | 类型安全 |
| Vite | 5.4 | 构建工具 |
| Zustand / Redux | - | 状态管理 |
| Framer Motion | - | 动画库 |
| D3.js | - | 知识图谱可视化 |
| ECharts | - | 统计图表 |
| KaTeX | - | 数学公式渲染 |
| TanStack Query | - | 数据获取 |

### 后端层

| 技术 | 版本 | 用途 |
|------|------|------|
| NestJS | 10 | API 框架 |
| TypeScript | 5.5 | 类型安全 |
| | | |

### AI 层

| 服务 | 提供商 | 用途 |
|------|--------|------|
| OpenAI | GPT-4 | 通用场景、代码生成 |
| Anthropic | Claude | 复杂推理、长文本 |
| DeepSeek | - | 成本优化、中文理解 |
| GLM | - | 国产模型支持 |

### 数据存储

| 数据库 | 用途 |
|--------|------|
| Neo4j | 知识图谱存储与查询 |
| Redis | 缓存、会话管理 |
| PostgreSQL | 用户数据（可选） |

---

## 项目结构

```
ahatutor/
├── src/
│   ├── frontend/                        # React 前端
│   │   ├── src/
│   │   │   ├── api/                    # API 客户端
│   │   │   │   └── agent.ts            # Agent API
│   │   │   ├── components/
│   │   │   │   ├── Visualization/      # 可视化组件
│   │   │   │   │   ├── KnowledgeGraph.tsx
│   │   │   │   │   ├── VisualDesignerView.tsx
│   │   │   │   │   └── NarrativeComposerView.tsx
│   │   │   │   ├── SpeedMode/          # 速通模式
│   │   │   │   ├── DepthMode/          # 深度模式
│   │   │   │   ├── MistakeBook/        # 错题本
│   │   │   │   └── Report/             # 学情报告
│   │   │   ├── pages/                  # 页面
│   │   │   └── App.tsx
│   │   └── package.json
│   │
│   ├── backend/                         # NestJS 后端
│   │   └── src/
│   │       ├── modules/
│   │       │   ├── agents/              # 六 Agent 流水线
│   │       │   │   ├── agent-pipeline.service.ts
│   │       │   │   ├── concept-analyzer.service.ts
│   │       │   │   ├── prerequisite-explorer.service.ts
│   │       │   │   ├── genetics-enricher.service.ts
│   │       │   │   ├── visual-designer.service.ts
│   │       │   │   ├── narrative-composer.service.ts
│   │       │   │   ├── quiz-generator.service.ts
│   │       │   │   ├── skills/          # Agent Skills
│   │       │   │   │   ├── web-search.service.ts
│   │       │   │   │   └── resource-recommend.service.ts
│   │       │   │   └── agent.controller.ts
│   │       │   ├── llm/                 # LLM 多管道
│   │       │   ├── rag/                 # RAG 框架
│   │       │   ├── knowledge-graph/     # 知识图谱
│   │       │   ├── mistake/             # 错题管理
│   │       │   └── report/              # 学情报告
│   │       ├── shared/
│   │       │   └── types/               # 共享类型定义
│   │       └── app.module.ts
│   │
│   └── shared/                          # 前后端共享代码
│       └── types/                       # 类型定义
│           ├── agent.types.ts
│           ├── genetics.types.ts
│           └── skill.types.ts
│
├── documents/                           # 教材文档
├── prompts/                             # Prompt 模板
├── docker-compose.yml                   # 本地开发环境
├── package.json
└── README.md
```

---

## 快速开始

### 环境要求

- Node.js 18+
- Redis（可选，用于缓存）
- Neo4j（可选，用于知识图谱）
- LLM API Key（OpenAI/Claude/DeepSeek 至少一个）

### 1. 克隆项目

```bash
git clone https://github.com/your-username/ahatutor.git
cd ahatutor
```

### 2. 安装依赖

```bash
# 安装后端依赖
cd src/backend
npm install

# 安装前端依赖
cd ../frontend
npm install
```

### 3. 配置环境变量

创建 `src/backend/.env`:

```env
# LLM 配置（至少配置一个）
DEFAULT_LLM_PROVIDER=openai
OPENAI_API_KEY=your_openai_key
CLAUDE_API_KEY=your_claude_key
DEEPSEEK_API_KEY=your_deepseek_key
GLM_API_KEY=your_glm_key

# 数据库配置
NEO4J_URI=bolt://localhost:7687
NEO4J_USER=neo4j
NEO4J_PASSWORD=ahatutor123

REDIS_HOST=localhost
REDIS_PORT=6379

# 网络搜索（可选）
WEB_SEARCH_PROVIDER=mock  # tavily | bing | mock
WEB_SEARCH_API_KEY=your_search_key
```

创建 `src/frontend/.env.local`:

```env
VITE_API_BASE_URL=http://localhost:3000
```

### 4. 启动数据库服务

```bash
# 启动 Redis 和 Neo4j（使用 Docker）
docker-compose up -d redis neo4j
```

### 5. 启动开发服务器

```bash
# 启动后端（端口 3000）
cd src/backend
npm run start:dev

# 启动前端（端口 5173）
cd src/frontend
npm run dev
```

### 6. 访问应用

打开浏览器访问: `http://localhost:5173`

---

## API 文档

### 六 Agent 流水线

```typescript
// 执行完整流水线
POST /agent/pipeline
{
  "concept": "伴性遗传",
  "userLevel": "intermediate",
  "focusAreas": ["quiz"]
}

// 快速分析
GET /agent/quick?concept=伴性遗传

// 生成学习路径
GET /agent/learning-path?concept=伴性遗传&userLevel=intermediate
```

### VisualDesigner

```typescript
// 设计可视化方案
POST /agent/visualize
{
  "concept": "伴性遗传",
  "includeEnrichment": true,
  "includePrerequisites": true
}

// 生成可视化代码
GET /agent/visualize/code?concept=伴性遗传
```

### NarrativeComposer

```typescript
// 创建学习叙事
POST /agent/narrative
{
  "concept": "伴性遗传",
  "userLevel": "intermediate"
}

// 生成详细学习脚本
POST /agent/narrative/script

// 生成互动式学习流程
POST /agent/narrative/interactive
```

### Agent Skills

```typescript
// 联网搜索
POST /agent/skills/search
{
  "query": "遗传学 伴性遗传 最新研究",
  "numResults": 5,
  "language": "zh"
}

// 推荐学习资源
POST /agent/skills/resources
{
  "concept": "伴性遗传",
  "userLevel": "intermediate",
  "preferredTypes": ["video", "article"],
  "count": 5
}
```

---

## 前端组件使用示例

```tsx
import { VisualDesignerView } from './components/Visualization/VisualDesignerView';
import { NarrativeComposerView } from './components/Visualization/NarrativeComposerView';
import { InteractiveFlowView } from './components/Visualization/NarrativeComposerView';

// 概念探索页面
function ConceptPage({ concept }) {
  return (
    <div className="grid grid-cols-2 gap-6">
      {/* 可视化展示 */}
      <VisualDesignerView
        concept={concept}
        userLevel="intermediate"
      />

      {/* 学习叙事 */}
      <NarrativeComposerView
        concept={concept}
        userLevel="intermediate"
      />
    </div>
  );
}

// 互动式学习流程
function InteractiveLearningPage({ concept }) {
  return (
    <InteractiveFlowView
      concept={concept}
      userLevel="beginner"
    />
  );
}
```

---

## 开发路线图

### ✅ Phase 1: 核心功能（已完成）

- [x] 六 Agent 流水线
- [x] RAG 框架
- [x] LLM 多管道
- [x] Agent Skills
- [x] 前后端 API 对接

### 🔄 Phase 2: 用户体验优化（进行中）

- [ ] 前端页面完善
- [ ] 移动端适配
- [ ] 性能优化
- [ ] 错误处理优化

### ⏳ Phase 3: 高级功能（计划中）

- [ ] 用户认证系统
- [ ] 学习进度持久化
- [ ] 艾宾浩斯复习计划
- [ ] 多模态支持（手写识别）

---

## 验收标准

### 功能验收

- ✅ 能基于遗传学知识回答基本问题
- ✅ 六 Agent 流水线完整运行
- ✅ 速通模式完整流程可运行
- ✅ 知识图谱正确展示概念关系
- ✅ 学习叙事生成连贯的故事

### 性能验收

| 指标 | 目标 |
|------|------|
| 流水线响应 | < 10s |
| 流式输出延迟 | < 500ms |
| 知识图谱渲染 | < 2s |
| 页面首屏加载 | < 3s |

---

## 贡献指南

欢迎贡献！请查看 [贡献指南](CONTRIBUTING.md)

---

## 许可证

MIT License - 详见 [LICENSE](LICENSE)

---

## 联系方式

- 项目主页: [https://github.com/your-username/ahatutor](https://github.com/your-username/ahatutor)
- 问题反馈: [Issues](https://github.com/your-username/ahatutor/issues)

---

**让每一次学习都成为"顿悟时刻"** ✨
