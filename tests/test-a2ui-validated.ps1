# A2UI Function Test Script (With Correct DTO)
...
$result1 = Test-Endpoint -Name "A2UI Visual Design" -Url "$API_BASE/visualize/ask" -Method POST -Body '{"concept":"Okazaki fragments","question":"What are Okazaki fragments?"}'
...
$testQuestions = @(
    @{"concept"="Okazaki fragments"; "question"="What are Okazaki fragments?"},
    @{"concept"="Hemophilia"; "question"="Show me the hemophilia pedigree chart"},
    @{"concept"="DNA replication"; "question"="How does DNA replication work?"}
)

foreach ($q in $testQuestions) {
    $questionJson = $q | ConvertTo-Json
    Write-Host "`nConcept: $($q.concept)"
...
try {
    $sseUrl = "$API_BASE/visualize/ask/stream?concept=test&question=test"
