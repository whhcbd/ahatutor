...
export class Neo4jService implements OnModuleInit, OnModuleDestroy {
   ...
   * 执行 Cypher 查询
   */
  async run(cypher: string, params: Record<string, any> = {}): Promise<Record[]> {
  ...
  async getNodeRelations(
    ...
    const cypher = `
      MATCH (n {id: $id})${directionPattern}[r]-(m)
      RETURN n as from, r as relationship, m as to
    `;
    const result = await this.run(cypher, { id });
   ...
   * 搜索节点
   */
  async searchNodes(query: string, limit = 10): Promise<Record[]> {
    const cypher = `
      MATCH (n)
  ...
  async getStats(): Promise<{
    ...
    for (const row of typeResult as any) {
      nodeTypes[row.type || 'unknown'] = row.count;
    }

    return {
      totalNodes: nodeResult[0]?.count || 0,
      totalEdges: edgeResult[0]?.count || 0,
      nodeTypes,
    };
  }
