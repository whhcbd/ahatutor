...
import { A2UI_TEMPLATES, buildA2UIPayload } from './data/a2ui-templates.data';
  ...
  private async generateVisualizationData(
    ...
    switch (vizType) {
      ...
      case 'probability_distribution':
        data = await this.generateProbabilityDistributionData(concept, undefined, enrichment, ragKnowledgePoints);
        break;
      case 'pedigree_chart':
...
返回 JSON 格式的遗传路径数据（必须包含从RAG知识库填充的知识点字段）。`;
  ...
  private async generateProbabilityDistributionData(
    ...
    const enrichmentInfo = enrichment
      ? `\n相关公式：${enrichment.formulas.map((f: any) => `${f.key}: ${f.latex}`).join('\n')}`
      : '';
    ...
    const prompt = `你是遗传学专家。请为"${concept}"生成一个概率分布的可视化数据。${enrichmentInfo}${questionInfo}
...
返回 JSON 数组格式。`;
  ...
  /**
   * 创建错误A2UI payload用于显示错误信息
   */
  private createErrorA2UIPayload(title: string, description: string, details?: any): any {
    return {
      version: '1.0',
      surface: {
        rootId: 'error-card',
  ...
  async answerQuestion(
    ...
    // 获取遗传学丰富化内容和RAG知识点
    let enrichment: GeneticsEnrichment | undefined;
    let ragKnowledgePoints: Record<string, any> = {};
    
    try {
        if (this.geneticsEnricher) {
          enrichment = await this.geneticsEnricher.enrichConcept(concept);
          this.logger.log(`Enriched concept: ${concept} with ${enrichment?.examples?.length || 0} examples`);
        }
        ...
        if (this.vectorRetrieval) {
          ...
          if (retrievalResult.success && retrievalResult.data) {
            for (const result of retrievalResult.data.results) {
              ragKnowledgePoints[result.metadata.chapter || 'general'] = result.content;
            }
            this.logger.log(`Retrieved ${retrievalResult.data.results.length} RAG knowledge points for ${concept}`);
          }
        }
      } catch (enrichmentError) {
        this.logger.warn('Failed to get enrichment or RAG knowledge points:', enrichmentError);
    }
...
- **多可视化请求处理**：如果用户要求"包含以下所有可视化"或类似的多可视化请求（如"请在一条回答中包含：Punnett方格、DNA结构、减数分裂等"）：
  - 识别所有请求的可视化类型
  - 在textAnswer中为每个可视化提供详细说明
  - 将needVisualization设为true
  - 在suggestedVisualizationType中使用逗号分隔的列表（如"punnett_square,dna_helix,meiosis,knowledge,gene_expression"）
  - 确保textAnswer中提到所有请求的可视化类型**
...
返回JSON格式。`;
    ...
    try {
      ...
      if (requiresVisualization) {
        ...
        try {
          ...
          if (templateMatch?.matched && templateMatch.template && templateMatch.confidence > 0.7) {
            ...
            let aiGeneratedParameters: Record<string, any> = {};

            if (templateMatch.template.visualizationType === 'punnett_square') {
              aiGeneratedParameters = await this.generatePunnettSquareDataWithQuestion(
                concept,
                question,
                enrichment,
                ragKnowledgePoints
              );
            } else if (templateMatch.template.visualizationType === 'probability_distribution') {
              aiGeneratedParameters = await this.generateProbabilityDistributionData(
                concept,
                question,
                enrichment,
                ragKnowledgePoints
              );
            } else if (templateMatch.template.visualizationType === 'inheritance_path') {
              aiGeneratedParameters = await this.generateInheritancePathDataWithQuestion(
                concept,
                question,
                enrichment,
                ragKnowledgePoints
            ...
            a2uiTemplate = buildA2UIPayload(templateMatch.template, aiGeneratedParameters);
      ...
      // 检测用户是否请求了多个可视化组件
      const multiVisualizationMatch = question.match(/(?:包含以下|以下所有|多条|多个).*?(?:可视化|Punnett方格|DNA结构|减数分裂|知识图谱|基因表达)/);
      if (multiVisualizationMatch) {
        this.logger.log('用户请求了多个可视化组件');

        const visualizationTypes = (response.suggestedVisualizationType || '').split(',').map(v => v.trim()).filter(v => v && v !== 'none');
        const a2uiTemplates: any[] = [];

        for (const vizType of visualizationTypes) {
          try {
            let matchedTemplate = null;
            for (const template of A2UI_TEMPLATES) {
              if (template.visualizationType === vizType || template.templateId.includes(vizType)) {
            ...
            if (matchedTemplate) {
              const templatePayload = buildA2UIPayload(matchedTemplate, matchedTemplate.defaultValues);
              a2uiTemplates.push(templatePayload);
              this.logger.log(`Generated A2UI for: ${vizType}`);
            }
          } catch (error) {
            this.logger.warn(`Failed to generate A2UI for ${vizType}:`, error);
          }
        }

        if (a2uiTemplates.length > 0) {
          return {
      ...
      // 检测用户是否请求了不存在的可视化组件（更严格的匹配，避免误匹配多可视化请求）
      const unsupportedComponentMatch = question.match(/(?:请展示|请显示|请生成|创建|绘制)\s+[a-zA-Z0-9-]+\s+可视化(?:组件|图表)?/);
      if (unsupportedComponentMatch) {
        const requestedComponent = unsupportedComponentMatch[0];
        this.logger.warn(`User requested unsupported visualization component: ${requestedComponent}`);

        // 返回错误A2UI payload
        a2uiTemplate = this.createErrorA2UIPayload(
          `不支持的可视化组件`,
          `您请求的可视化组件 "${requestedComponent}" 不存在或暂不支持。`,
          {
            requestedComponent,
            availableComponents: ['punnett_square', 'dna_helix', 'inheritance_path', 'knowledge_graph', 'probability_distribution']
          }
        );
      }

      if (!a2uiTemplate) {
        this.logger.log(`No template match or confidence too low, skipping visualization (A2UI only mode)`);
      }
  ...
  private async generateQuestionBasedVisualization(
    ...
    try {
      ...
      switch (vizType) {
        ...
        case 'probability_distribution':
          const probData = await this.generateProbabilityDistributionData(concept, undefined, undefined, ragKnowledgePoints);
          return {
