...
5. **数学公式必须使用 LaTeX 格式**：
   - 所有数学公式必须使用 LaTeX 语法，不要使用 Unicode 字符
   - 使用 $...$ 表示行内公式（例如：$P(AA) = \frac{1}{4}$）
   - 使用 $$...$$ 表示独立公式（例如：$$(A + a)^2 = A^2 + 2Aa + a^2$$）
   - 分数使用 \frac{分子}{分母} 格式（例如：\frac{1}{4} 而不是 1/4）
   - 指数使用 ^ 符号（例如：A^2 而不是 A²）
   - 下标使用 _ 符号（例如：F_2 而不是 F₂）
   - 绝对不要使用 Unicode 数学符号（如 ²、³、₁、₂、₃ 等）
...
**数学公式格式要求：**
- 所有数学公式必须使用 LaTeX 格式，不要使用 Unicode 字符
- 使用 $...$ 表示行内公式（例如：$P(AA) = \frac{1}{4}$）
- 使用 $$...$$ 表示独立公式（例如：$$(A + a)^2 = A^2 + 2Aa + a^2$$）
- 分数使用 \frac{分子}{分母} 格式（例如：\frac{1}{4} 而不是 1/4）
- 指数使用 ^ 符号（例如：A^2 而不是 A²）
- 下标使用 _ 符号（例如：F_2 而不是 F₂）
- 不要使用 Unicode 数学符号（如 ²、³、₁、₂、₃ 等）`;
