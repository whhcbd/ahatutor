...
const TOPIC_CATEGORIES = [
  { name: '孟德尔遗传学', topics: ['孟德尔定律', '孟德尔第一定律', '孟德尔第二定律', '伴性遗传', '基因型与表型', 'X连锁遗传', 'Y连锁遗传', '连锁互换', '交叉互换', '联会', '四分体', '质量性状', '数量性状', '多基因遗传', '从性遗传', '限性遗传'] },
...
const TOPICS_BY_COMPLEXITY = {
  basic: ['孟德尔定律', '孟德尔第一定律', '基因型与表型', '中心法则', '密码子', '反密码子', '起始密码子', '终止密码子'],
  intermediate: ['孟德尔第二定律', '伴性遗传', 'X连锁遗传', 'Y连锁遗传', '连锁互换', '交叉互换', '联会', '四分体', '质量性状', '数量性状', '减数分裂', 'DNA复制', 'DNA半保留复制', '复制叉', '前导链', '后随链', '冈崎片段', '转录', '翻译', '基因频率', '基因型频率'],
  advanced: ['染色体畸变', '非整倍体', '三体', '单体', '染色体结构畸变', '缺失', '重复', '倒位', '易位', '多基因遗传', '从性遗传', '限性遗传', '基因调控', '操纵子', '乳糖操纵子', '基因突变', '点突变', '移码突变', '同义突变', '错义突变', '无义突变', 'DNA修复', '哈代-温伯格定律', '遗传平衡', '遗传漂变', '自然选择', '表观遗传学'],
};
...
export default function VisualizePage() {
  ...
  // 获取硬编码概念列表
  useEffect(() => {
    agentApi.getHardcodedConcepts().then(setHardcodedConcepts).catch(console.error);
  }, []);
